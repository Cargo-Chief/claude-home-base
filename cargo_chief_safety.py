...
def _build_harness_prompt(
    ...
    *,
    ...
    return (
        ...
        "- To delegate, write exactly one JSON object with `tier`, `prompt`, and boolean "
        "`mutation` keys to "
        f"{delegation_request_file}. Allowed tiers are implementation, bounded, mechanical, and "
        "explore. When a coordinator supplies `budget_unit` and `planned_tokens`, copy both keys "
        "exactly into that object; the supported unit is `generation_tokens_v1`. Do not invent "
